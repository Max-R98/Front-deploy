...
function Booking() {
  ...
  useEffect(() => {
    ...
    fetch(`https://back-deploy-production-f20c.up.railway.app/users/profile/${userEmail}`, {
      headers: {
    ...
    // Fetch del producto
    fetch(`https://back-deploy-production-f20c.up.railway.app/productos/${id}`)
      .then((response) => {
        if (!response.ok) {
  ...
  const confirmarReserva = async () => {
    try {
      ...
      const response = await fetch("https://back-deploy-production-f20c.up.railway.app/reservas/crear", {
        method: "POST",
        headers: {
