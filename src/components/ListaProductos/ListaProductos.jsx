...
function ListaProductos() {
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch("https://back-deploy-production-f20c.up.railway.app/productos/listar");
      if (!response.ok) {
  ...
  const fetchCategories = async () => {
    try {
      const response = await fetch("https://back-deploy-production-f20c.up.railway.app/categorias/listar");
      if (!response.ok) {
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      const response = await axios.put(
        `https://back-deploy-production-f20c.up.railway.app/productos/editar`,
        {
  ...
  const handleDeleteProduct = async () => {
    try {
      await axios.delete(
        `https://back-deploy-production-f20c.up.railway.app/productos/eliminar/${productToDelete}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
